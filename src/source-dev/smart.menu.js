...
Smart('smart-menu', class Menu extends Smart.BaseElement {
    ...
    propertyChangedHandler(propertyName, oldValue, newValue) {
        ...
        switch (propertyName) {
            ...
            case 'mode': {
                ...
                const menuItemsGroups = that.enableShadowDOM ? that.shadowRoot.querySelectorAll('smart-menu-items-group') : that.getElementsByTagName('smart-menu-items-group');
    ...
    _createItemHTMLStructure(currentItem, level, item, index) {
        ...
        if (level > 1) {
            ...
            if (that.checkboxes && item.checkable) {
                if (item.checkMode === 'checkbox') {
            ...
            if (that.checkboxes && that.checkable) {
                if (that.checkMode === 'checkbox') {
                    currentItem.setAttribute('role', 'menuitemcheckbox');
                }
                else {
                    currentItem.setAttribute('role', 'menuitemradio');
    ...
    _transitionendHandler(event) {
        ...
        if (that.mode === 'dropDown' && that.opened && event.target === that &&
            that.getRootNode().activeElement !== that && !that._noAutoFocus) {
            that.focus();
        }
    }
