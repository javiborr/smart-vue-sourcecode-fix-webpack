    ...
    */
    ...
    insert(item, path) {
        ...
        function createTreeItem(item) {
            const subItems = item[that.itemsMember] || [],
                tagName = item.tagName === 'smart-tree-items-group' || subItems.length > 0 ? 'smart-tree-items-group' : 'smart-tree-item',
    ...
    _scrollViewerDownHandler(event) {
        ...
        if (that._isMobile) {
            ...
            that._downTimeoutInfo = { target: target, event: event, scrollTop: current, timeout: timeout };
        }
        else {
            that._continueSelection(target, event);
        }
